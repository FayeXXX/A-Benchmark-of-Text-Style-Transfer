...
def run(*argv):
	...
	if args.dev0 is None and args.dev1 is None and args.test0 is None and args.test1 is None:
		args.test0 = f"../outputs/{cargs.name}/neg2pos.txt"
		args.test1 = f"../outputs/{cargs.name}/pos2neg.txt"
...
def eval_main(args):
	...
	import run_cls
	cls_param = Storage()
	cls_param.args = run_cls.run("--dryrun", "--restore", args.clsrestore, "--cuda", "--dataid", \
								 args.dataid, "--roberta", "/home/xyf/LanguageModel/roberta-base", \
								 "--model_dir", "../styletransformer/model/")
	...
	lm_param.args = run_lm.run("--dryrun", "--restore", args.lmrestore, "--dataid", args.dataid, \
							   "--gpt","/home/xyf/LanguageModel/gpt2", \
							   "--model_dir", "../styletransformer/model/")
