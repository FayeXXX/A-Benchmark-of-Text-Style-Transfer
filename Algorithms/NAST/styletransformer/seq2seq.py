...
class Seq2seq(BaseModel):
	...
	def train_process(self):
		...
		while self.now_epoch < args.epochs:
			...
			if self.now_epoch % 10 == 0:
				...
				testloss_detail = self.evaluate("test", hasref=False, write_output=True)
				...
				if flags[0]:
					shutil.copyfile(f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_batch:{self.now_batch}.0",
									f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_best.0")
				if flags[1]:
					shutil.copyfile(f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_batch:{self.now_batch}.1",
									f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_best.1")
	...
	def pretrain(self, batch_num):
		...
		print(f'pretrain_batch:{batch_num}')
		...
		for i in range(batch_num):
			...
			slf_loss, cyc_loss, _, slf_length_loss, _, slf_gen_error, _ = self.f_step(incoming, temperature=1.0, cyc_rec_enable=True, use_inference_z=True)
	...
	def evaluate(self, datakey, hasref, write_output):
		...
		def inference(datakey, domain):
			...
			show_str = []
			for i in range(len(result["sgen"])):
		...
		if not os.path.exists(f'{self.args.out_dir}/{self.args.dataset}'):
			os.mkdir(f'{self.args.out_dir}/{self.args.dataset}')
		if write_output:
			with open(f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_batch:{self.now_batch}.0", 'w') as f:
				for sent in result0["gen"]:
					f.write(' '.join(sent) + "\n")

			with open(f"{self.args.out_dir}/{self.args.dataset}/{self.args.name}_batch:{self.now_batch}.1", 'w') as f:
