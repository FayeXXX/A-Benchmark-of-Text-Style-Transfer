...
import torch
import argparse
from prompt_utils import DATASET_TO_INSTRUCTION_MAPPING
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['GYAFC_FR_INFORMAL', 'GYAFC_FR_FORMAL', 'YELP-NEGATIVE', 'YELP-POSITIVE',
                                              'GYAFC_EM_INFORMAL', 'GYAFC_EM_FORMAL', 'AMAZON-NEGATIVE', 'AMAZON-POSITIVE',
...
def main(
    ...
    total = len(dataset)
    if args.adapter in ['LORA', 'SALORA', 'ADALORA', 'PROMPT_TUNING', 'PREFIX_TUNING']:
        exp_dir = os.path.join("./experiment", f"{args.dataset}", get_last_two_levels(args.lora_weights))
...
def load_data(args) -> list:
    ...
    sentences = []
    ref_one = ["AMAZON-NEGATIVE", "AMAZON-POSITIVE", "SHAKESPEARE-MODERN", "SHAKESPEARE-ORIGINAL", "PTB-FUTURE", "PTB-REMOVAL", "YELP-NEGATIVE", "YELP-POSITIVE"]
    if any(args.dataset.startswith(prefix) for prefix in ref_one):
