...
def train(
    ...
    dataset: str = "styleptb_ARR ",
):
    print(
        f"Training {adapter_name} model with params:\n"
    ...
    if "GYAFC" in dataset:
        data = load_dataset('json', data_files={"train": f"TST_data/new_data/train/{dataset}.json",
                                                "dev": f"TST_data/new_data/test/{dataset}.json"})
    else:
        data = load_dataset('json', data_files={"train": f"TST_data/new_data/train/{dataset}.json",
                                                "dev": f"TST_data/new_data/dev/{dataset}.json"})
    ...
    if val_set_size > 0:
        ...
        val_data = (
            data["dev"].shuffle().map(generate_and_tokenize_prompt)
        )
