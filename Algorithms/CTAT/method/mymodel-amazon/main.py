...
#  File parameters
######################################################################################
parser.add_argument('--dataset', type=str, default='shakespeare', help='Specify datasets.')
...
def preparation():
    ...
    args.current_save_path = f'save/{args.dataset}/'
    args.log_file_path = args.current_save_path + 'log/'
    if not os.path.exists(args.current_save_path):
        os.mkdir(args.current_save_path)
    if not os.path.exists(args.log_file_path):
        os.mkdir(args.log_file_path)
    args.log_file = args.log_file_path + time.strftime("log_%Y_%m_%d_%H_%M_%S.txt", time.localtime())
    ...
    if args.dataset == 'yelp':
        ...
        args.data_path = '../../data/imdb/processed_files/'
    elif args.dataset == 'gyafcem':
        args.data_path = '../../data/gyafcem/processed_files/'
    elif args.dataset == 'gyafcfr':
        args.data_path = '../../data/gyafcfr/processed_files/'
    elif args.dataset == 'shakespeare':
        args.data_path = '../../data/shakespeare/processed_files/'
    else:
    ...
    args.train_file_list, args.train_label_list = prepare_data(
        data_path=args.data_path, max_num=args.word_dict_max_num, task_type=args.dataset
    )
...
def train_iters(ae_model, dis_model):
    ...
    # ae_optimizer = NoamOpt(ae_model.src_embed[0].d_model, 2, 4000,
    #                        torch.optim.Adam(ae_model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
    dis_optimizer = torch.optim.Adam(dis_model.parameters(), lr=args.learning_rate)

    ae_criterion = get_cuda(LabelSmoothing(size=args.vocab_size, padding_idx=args.id_pad, smoothing=args.smooth))
    ...
    for epoch in range(200):
        ...
        # Save model
        # if epoch % 10 == 0:
    torch.save(ae_model.state_dict(), f'{args.current_save_path}ae_model_params_{epoch}.pkl')
    torch.save(dis_model.state_dict(), f'{args.current_save_path}dis_model_params_{epoch}.pkl')
    output_file = args.current_save_path + f'dropout:{str(args.dropout)}_smooth:{str(args.smooth)}_epoch:{epoch}.txt'
    eval_iters(ae_model, dis_model, output_file)
    return


def eval_iters(ae_model, dis_model, output_file):
    eval_data_loader = non_pair_data_loader(
        batch_size=1, id_bos=args.id_bos,
        id_eos=args.id_eos, id_unk=args.id_unk,
        max_sequence_length=args.max_sequence_length, vocab_size=args.vocab_size
    )
    if args.dataset == 'yelp' or args.dataset == 'amazon' or args.dataset == 'imdb':
        eval_file_list = [
    ...
    elif args.dataset == 'gyafcem' or args.dataset == 'gyafcfr' or args.dataset == 'shakespeare':
        eval_file_list = [
    ...
    gold_ans = load_human_answer(args.data_path, args.dataset)
    ...
    for it in range(eval_data_loader.num_batch):
        ...
        add_output("\ngold: " + id2text_sentence(gold_ans[it], args.id_to_word), output_file)
        modify_text = fgim_attack(dis_model, latent, target, ae_model, args.max_sequence_length, args.id_bos,
                                        id2text_sentence, args.id_to_word, gold_ans[it],output_file)
