...
from Model_cyclic import Model
...
def train_process(model, train_data):
    ...
    while train_epoch < global_config.num_epochs:
        ...
        if running_log_name is None:
            running_log_name = "./cyclic_running_log/" + time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()) + "_" + str(running_random_number) + ".txt"
...
def evaluate_process(model, data_test_collection, train_epoch, best_score=None, preview=False, fast_infer=False):
    ...
    for tmp_i in range(len(data_test_collection)):
        ...
        if global_config.print_all_predictions:
            with open("./cyclic_saved_models/" + str(running_random_number) + "_epoch" + str(train_epoch) + "_all_generation.txt", "w", encoding="utf-8") as fp:
    ...
    if global_config.save_model and global_config.train and train_epoch >= 1:
        model.save_model("./cyclic_saved_models/best_model_" + str(running_random_number) + "_epoch" + str(train_epoch) + "_" + str(current_eval_score)[:7] + ".pth")
