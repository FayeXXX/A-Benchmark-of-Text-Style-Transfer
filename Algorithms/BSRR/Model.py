    ...
    def rewrite_sentence_RL(self, batch_input_tensor, batch_attention_mask, greedy_generation=True, plus_sampling_generation=False, set_min_length=7, set_max_length=25):
        ...
        if plus_sampling_generation:
            output_sampling_sen_token_ids, output_sampling_sen_token_logits = \
                self.language_backbone.generate(input_ids=batch_input_tensor, attention_mask=batch_attention_mask,
                                                output_scores=False, return_dict_in_generate=False,
                                                do_sample=True, num_beams=1, early_stopping=False,
                                                min_length=set_min_length, max_length=set_max_length, use_cache=False)
...
class StyleClassifier(nn.Module):
    def __init__(self, corpus_mode):
        ...
        print("Loading BERT model for style classification:", corpus_mode)
        ...
        self.style_classifier.load_state_dict(torch.load('saved_models/TextBERT_' + self.corpus_mode + '/TextBERT_best.chkpt'))
    ...
    def binary_cls(self, batch_text, batch_label=None):
        ...
        if self.corpus_mode in ["amazon", "Yelp", "GYAFC_FR", "GYAFC_EM", "shakespeare", "styleptb_ARR", "styleptb_TFU"]:
...
class Model:
    def __init__(self, corpus_mode):
        ...
        self.style_CLS = StyleClassifier(corpus_mode)
        if global_config.use_cuda:
            self.style_CLS.cuda()
        ...
        self.style_class_dict = {"informal": 0, "formal": 1, "negative": 0, "positive": 1, "shakespeare": 0, "modern": 1,
                                 "origin": 0, "remove adjectives and adverbs": 1, "convert it into the future tense": 1}

        if corpus_mode in ["amazon", "Yelp"]:
        ...
        elif corpus_mode in["GYAFC_FR", "GYAFC_EM", "shakespeare", "styleptb_ARR", "styleptb_TFU"]:
    ...
    def forward(self, batch, eval_mode=False):
        ...
        batch_target_style_list = [i.split("#")[0].strip().replace("<s>", "") for i in batch_supervised_target_text]
    ...
    def batch_train(self, batch, epoch_number, args):
        self.agent.train()
        ...
        if self.RL_training is False:
            ...
            (supervised_loss + args.cyclic * cyclic_loss + args.style * GAN_gen_loss).backward()
