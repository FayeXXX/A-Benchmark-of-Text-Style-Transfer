...
def main():
    ...
    elif corpus_name in ["shakespeare"]:
        with open(corpus_name + '_data/train.0', 'r') as f:
            tmp = f.readlines()
            train_0 = [[i.strip().lower(), 0] for i in tmp]
        with open(corpus_name + '_data/train.1', 'r') as f:
            tmp = f.readlines()
            train_1 = [[i.strip().lower(), 1] for i in tmp]
        with open(corpus_name + '_data/test.0', 'r') as f:
            tmp = f.readlines()
            valid_0 = [[i.strip().lower(), 0] for i in tmp]
        with open(corpus_name + '_data/test.1', 'r') as f:
            tmp = f.readlines()
            valid_1 = [[i.strip().lower(), 1] for i in tmp]
    ...
    elif corpus_name in ["styleptb_ARR", "styleptb_TFU"]:
        with open(f'styleptb/{corpus_name.split("_")[1]}/train.0', 'r') as f:
            tmp = f.readlines()
            train_0 = [[i.strip().lower(), 0] for i in tmp]
        with open(f'styleptb/{corpus_name.split("_")[1]}/train.1', 'r') as f:
            tmp = f.readlines()
            train_1 = [[i.strip().lower(), 1] for i in tmp]
        with open(f'styleptb/{corpus_name.split("_")[1]}/test.0', 'r') as f:
            tmp = f.readlines()
            valid_0 = [[i.strip().lower(), 0] for i in tmp]
        with open(f'styleptb/{corpus_name.split("_")}/test.1', 'r') as f:
            tmp = f.readlines()
            valid_1 = [[i.strip().lower(), 1] for i in tmp]
        ...
        for idx, batch in enumerate(train_batches):
            ...
            if train_steps % opt.eval_step == 0:
                ...
                if avg_acc < valid_acc or True:
                    ...
                    if not os.path.exists('saved_models/TextBERT_' + corpus_name):
                        os.mkdir('saved_models/TextBERT_' + corpus_name)
