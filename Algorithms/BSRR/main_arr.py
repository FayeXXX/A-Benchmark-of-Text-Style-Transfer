...
parser.add_argument('--dataset', type=str, default='styleptb_ARR')
...
elif global_config.corpus_mode in ["styleptb_ARR", "styleptb_TFU"]:
    """ Reading styleptb data """
    train_sample, test_sample = [], []
    ptb = global_config.corpus_mode.split("_")[-1]

    """ reading amazon training sample version 2.0: add post processing """
    list_a = open(f"styleptb/{ptb}/train.0", "r", encoding="utf-8").readlines()
    list_b = open(f"styleptb/{ptb}/train.1", "r", encoding="utf-8").readlines()

    assert len(list_a) == len(list_b)

    for i in range(len(list_a)):
        train_sample.append(
            (process_lines([list_a[i], ], class_type=0, prefix_A="origin", prefix_B="remove adjectives and adverbs")[0],
             process_lines([list_b[i], ], class_type=1, prefix_A="origin", prefix_B="remove adjectives and adverbs")[0]))

    list_a = open(f"styleptb/{ptb}/test.0", "r", encoding="utf-8").readlines()
    list_b = open(f"styleptb/{ptb}/test.1", "r", encoding="utf-8").readlines()
...
def evaluate_process(model, data_test_collection, train_epoch, best_score=None, preview=False, fast_infer=False):
    ...
    for tmp_i in range(len(data_test_collection)):
        ...
        elif global_config.corpus_mode in ["amazon", "styleptb_ARR", "styleptb_TFU"]:
            """ Using one reference """
            all_gold_sentences = [[word_tokenize(v.replace("</s>", "").split("tive # ")[-1]), ] for k, v in enumerate(all_gold_sentences)]
