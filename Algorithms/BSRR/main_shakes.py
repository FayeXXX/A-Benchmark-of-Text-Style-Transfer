...
# MODE_MAP = {"yelp":"Yelp", "amazon":"amazon", "gyafc_em":"GYAFC_EM", "gyafc_fr":"GYAFC_FR", "shakespeare":"shakespeare"}
...
def process_lines(text_lines, class_type, prefix_A, prefix_B):
    tmp_lines = [re.sub("\s+", " ", i.replace("\t", " ")).strip()[:200] for i in text_lines]
...
if global_config.corpus_mode == "Yelp":
    ...
    train_sample, test_sample = [], []
    train_data_files = {"GYAFC_data/Family_Relationships/train/informal": "GYAFC_data/Family_Relationships/train/formal"}

    for i in train_data_files.keys():
        train_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=0, prefix_A="formal", prefix_B="informal"),
                process_lines(open(train_data_files[i], encoding="utf-8").readlines(), class_type=1, prefix_A="formal", prefix_B="informal")))

    """ Reading test samples from informal -> formal """
    test_data_files = {"GYAFC_data/Family_Relationships/test/informal": "GYAFC_data/Family_Relationships/test/formal.ref0"}

    for i in test_data_files.keys():
        test_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=0, prefix_A="formal", prefix_B="informal"),
                process_lines(open(test_data_files[i], encoding="utf-8").readlines(), class_type=1, prefix_A="formal", prefix_B="informal")))
...
elif global_config.corpus_mode == "GYAFC_EM":
    """ Reading GYAFC data with formality label """
    print("Reading GYAFC data with formality label")
    ...
    for i in train_data_files.keys():
        train_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=0, prefix_A="formal",
                              prefix_B="informal"),
                process_lines(open(train_data_files[i], encoding="utf-8").readlines(), class_type=1,
    ...
    for i in test_data_files.keys():
        test_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=0, prefix_A="formal",
                              prefix_B="informal"),
                process_lines(open(test_data_files[i], encoding="utf-8").readlines(), class_type=1, prefix_A="formal",
                              prefix_B="informal")))

    multi_candidate_list = []
    fp_list = [open("GYAFC_data/Entertainment_Music/test/formal.ref" + str(i), encoding="utf-8").readlines() for i in
               range(4)]
    for i in range(len(fp_list[0])):
        multi_candidate_list.append([fp_list[j][i] for j in range(4)])
...
elif global_config.corpus_mode == "shakespeare":
    """ Reading shakespeare data """
    print("Reading shakespeare data")

    train_sample, test_sample = [], []
    train_data_files = {"shakespeare_data/train.0": "shakespeare_data/train.1"}

    for i in train_data_files.keys():
        train_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=0, prefix_A="modern", prefix_B="shakespeare"),
                process_lines(open(train_data_files[i], encoding="utf-8").readlines(), class_type=1, prefix_A="modern", prefix_B="shakespeare")))

    """ Reading test samples from shakespeare -> modern """
    test_data_files = {"shakespeare_data/test.0": "shakespeare_data/test.1",
                       "shakespeare_data/test.1": "shakespeare_data/test.0"}

    for i in test_data_files.keys():
        test_sample.extend(
            zip(process_lines(open(i, encoding="utf-8").readlines(), class_type=int(i[-1]),
                              prefix_A="modern", prefix_B="shakespeare"),
                process_lines(open(test_data_files[i], encoding="utf-8").readlines(), class_type=int(test_data_files[i][-1]),
                              prefix_A="modern", prefix_B="shakespeare")))
...
def evaluate_process(model, data_test_collection, train_epoch, best_score=None, preview=False, fast_infer=False):
    ...
    for tmp_i in range(len(data_test_collection)):
        ...
        elif global_config.corpus_mode == "amazon" or global_config.corpus_mode == "shakespeare":
            """ Using one reference """
            all_gold_sentences = [[word_tokenize(v.replace("</s>", "").split("tive # ")[-1]), ] for k, v in enumerate(all_gold_sentences)]

        elif "GYAFC" in global_config.corpus_mode:
            # """ Using multiple reference """
