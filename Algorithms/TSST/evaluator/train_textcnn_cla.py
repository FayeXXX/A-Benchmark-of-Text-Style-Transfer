...
from utils.dataset import SCIterator, LABEL_MAP
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained('/home/xyf/LanguageModel/bert-base-uncased')

    train_src, train_tgt, valid_src, valid_tgt = [], [], [], []
    if os.path.exists('../data/{}/train.0.pkl'.format(opt.dataset)) and os.path.exists('../data/{}/train.1.pkl'.format(opt.dataset)) and os.path.exists(
            '../data/{}/dev.0.pkl'.format(opt.dataset)) and os.path.exists('../data/{}/dev.1.pkl'.format(opt.dataset)):
        train_src = pk.load(open('../data/{}/train.0.pkl'.format(opt.dataset), 'rb'))
        train_tgt = pk.load(open('../data/{}/train.1.pkl'.format(opt.dataset), 'rb'))
        valid_src = pk.load(open('../data/{}/dev.0.pkl'.format(opt.dataset), 'rb'))
        valid_tgt = pk.load(open('../data/{}/dev.1.pkl'.format(opt.dataset), 'rb'))
    else:
        ...
        with open('../data/{}/dev.0'.format(opt.dataset),'r') as f:
            for line in f.readlines():
                valid_src.append(tokenizer.encode(line.strip())[:opt.max_len])
        with open('../data/{}/dev.1'.format(opt.dataset),'r') as f:
            for line in f.readlines():
                valid_tgt.append(tokenizer.encode(line.strip())[:opt.max_len])
        pk.dump(train_src, open('../data/{}/train.0.pkl'.format(opt.dataset), 'wb'))
        pk.dump(train_tgt, open('../data/{}/train.1.pkl'.format(opt.dataset), 'wb'))
        pk.dump(valid_src, open('../data/{}/dev.0.pkl'.format(opt.dataset), 'wb'))
        pk.dump(valid_tgt, open('../data/{}/dev.1.pkl'.format(opt.dataset), 'wb'))
    ...
    embedding_path = os.path.join(os.path.join(cur_dir, "classifier"), "{}/textcnn/embedding.pt".format(opt.dataset))
    if os.path.exists(embedding_path):
        embedding = torch.load(embedding_path)
    else:
        embed_path = '/home/xyf/LanguageModel/glove.840B.300d.txt'
    ...
    # model.load_state_dict(torch.load("../checkpoints/textcnn_fr.chkpt"))
    # model.eval()
    # valid_acc, valid_loss = evaluate_sc(model, valid_loader, loss_fn, 0)
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if optimizer.steps % opt.eval_step == 0:
                ...
                if avg_acc < valid_acc:
                    avg_acc = valid_acc
                    save_path = './classifier/{}/textcnn/textcnn.chkpt'.format(opt.dataset)
