...
def main():
    ...
    parser.add_argument('-lr', default=3e-5, type=float, help='the learning rate')
    ...
    parser.add_argument('-dataset', default='shakespeare', type=str, help='the name of dataset')
    ...
    parser.add_argument('-patience', default=6, type=int, help='early stopping fine-tune')
    parser.add_argument('-seed', default=42, type=int, help='pseudo random generator seed')
    parser.add_argument('-log_step', default=10, type=int, help='print logs every x step')
    ...
    parser.add_argument('-ce_factor', default=1.2, type=float, help='proportion of data')
    ...
    base = BartModel.from_pretrained("/home/xyf/LanguageModel/bart-large")
    model = BartForMaskedLM.from_pretrained('/home/xyf/LanguageModel/bart-large', config=base.config)
    ...
    for step in range(1, opt.steps):
        ...
        loss_ce = opt.ce_factor * loss_fn(shift_logits.view(-1, shift_logits.size(-1)),
                          shift_labels.view(-1))
        ...
        if opt.sc and (200 < step or len(train_loader) < step):
            ...
            loss_sc = cal_sc_loss(logits, idx, cls, tokenizer, opt.style, opt.sc_factor)
            total_loss_sc.append(loss_sc.item())
        if opt.bl and (200 < step or len(train_loader)< step):
            idx = tgt.ne(tokenizer.pad_token_id).sum(-1)
            loss_co = cal_bl_loss(logits, tgt, idx, tokenizer, opt.bl_factor)
        ...
        if ((len(train_loader) > opt.eval_step
            ...
            if eval_loss >= valid_loss:
                ...
                # torch.save(model.state_dict(), 'checkpoints/{}_{}_{}_{}_step_{}.chkpt'.format(
                #         opt.model, opt.dataset, opt.order, opt.style, step))
                if not os.path.exists(f'checkpoints/{opt.model}_{opt.dataset}/'):
                    os.mkdir(f'checkpoints/{opt.model}_{opt.dataset}/')
                torch.save(model.state_dict(), 'checkpoints/{}_{}/sc:{}_bl:{}_{}_step_{}.chkpt'.format(
                    opt.model, opt.dataset, opt.sc_factor, opt.bl_factor, opt.style, step))
                shutil.copy('checkpoints/{}_{}/sc:{}_bl:{}_{}_step_{}.chkpt'.format(opt.model, opt.dataset, opt.sc_factor, opt.bl_factor, opt.style, step),
                            'checkpoints/{}_{}/sc:{}_bl:{}_{}.chkpt'.format(opt.model, opt.dataset, opt.sc_factor, opt.bl_factor, opt.style))
                print('[Info] The checkpoint file has been updated.')
                print('opt.order:',opt.order)
                print('checkpoints/{}_{}/sc:{}_bl:{}_{}_step_{}.chkpt'.format(opt.model, opt.dataset, opt.sc_factor, opt.bl_factor, opt.style, step))
